...
<form action="{{ route('product.update', $product->id) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="flex items-center gap-6 mobile:gap-4">
        ...
        <div class="desktop:hidden text-lg text-slate-700 font-medium">Edit Produk</div>
    ...
    <div class="flex mobile:flex-col items-start gap-10 mobile:gap-6 mt-10">
        <div id="image_area" class="bg-slate-300 rounded-xl w-96 p-4 mobile:w-full aspect-square bg-cover relative flex flex-col gap-4 items-end justify-start" style="background-image: url({{ asset('storage/product_images/' . $product->image) }})">
            ...
            <div class="flex items-center gap-2 bg-white rounded-full p-2 px-4">
                <ion-icon name="image-outline" class="text-sm text-slate-700"></ion-icon>
                <div class="text-xs text-slate-600">Ganti Gambar</div>
            </div>
        ...
        <div class="flex flex-col gap-4 grow mobile:w-full">
            <div class="group border border-slate-300 focus-within:border-blue-500 rounded-lg p-1 relative px-3">
                ...
                <input type="text" name="name" id="name" value="{{ $product->name }}" class="w-full outline-0 h-12 text-sm bg-transparent mt-3" required />
            </div>
            ...
            <div class="group border border-slate-300 focus-within:border-blue-500 rounded-lg p-1 relative px-3 flex items-center gap-4">
                ...
                <input type="number" value="{{ $product->stock }}" min="1" name="stock" id="stock" class="w-full outline-0 h-12 text-sm bg-transparent mt-3" required />
            ...
            <div class="group border border-slate-300 focus-within:border-blue-500 rounded-lg p-1 relative px-3">
                ...
                <input type="text" name="price" id="price" class="w-full outline-0 h-12 text-sm bg-transparent mt-3" oninput="typePrice(this)" value="{{ currency_encode($product->price) }}" required />
            </div>
        </div>
    </div>
</form>
